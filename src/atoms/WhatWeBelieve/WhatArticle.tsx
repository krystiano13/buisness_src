...
const WhatArticle:React.FC<WhatArticleInterface> = ({ mirrorPlacement, firstTitle, secondTitle, imageID }) => {
    return (
      <div className={ mirrorPlacement === false ? "WhatArticle" : "WhatArticle reverse"}>
        <section className="WhatArticle__first">
          ...
          <button className="WhatArticle__first__button">LEARN MORE</button>
        </section>
        <section className={ mirrorPlacement === false ? "WhatArticle__second" : "WhatArticle__second reverseFlex"}>
            <img src={imageID === 1 ? bg1 : bg2} className="WhatArticle__second__image" alt='background Image' />
        </section>
      </div>
    );
}
